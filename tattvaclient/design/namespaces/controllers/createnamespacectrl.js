angular.module('tattva')
    .controller("createNamespaceCtrl", ["$rootScope", "$scope", "AuthService",
        "$state", "$http", "$mdDialog", "namespaceFactory", "$stateParams",
        "jsonFilter",
        function($rootScope, $scope, AuthService, $state, $http, $mdDialog,
            namespaceFactory, $stateParams, jsonFilter) {
            $scope.nameSpace = {};
            ...
            $scope.autoExpand = function(e) {
                var element = typeof e === 'object' ? e.target : document.getElementById(
                    e);
            ...
            if ($stateParams.editNamespaceData) {
                // var element = document.getElementById('jsonSample');
                        ...
                        if ($scope.nameSpace.dataSchema[i].type == 'time') {
                            if (!moment(sample, moment.ISO_8601).isValid()) {
                                $mdDialog.show(
                                    ...
                                    .textContent(
                                        'Follow Valid Time Format (format: moment.ISO_8601)')
                                    .ariaLabel('Parsing Error.')
                                    .ok('Ok')
                                    .targetEvent(ev)
                                );
            ...
            $scope.createNamespaceSubmit = function(ev) {
                if ($stateParams.editNamespaceData) { //edit namespace page - save button
                    ...
                    if ($scope.createNameSpace.$dirty) { //if form elements are changed in edit page
                        if ($scope.createNameSpace.$valid) {
                            ...
                            var confirm = $mdDialog.confirm()
                                .title('Are you sure you want to change namespace ' +
                            ...
                            $mdDialog.show(confirm).then(function() {
                                ...
                                arr.push("on");
                                arr.push(moment().startOf($scope.nameSpace.editedOn).format(
                                    'MMMM Do YYYY, h:mm:ss a'));
                                //console.log(arr);
                                $rootScope.socket1.emit('notification', arr);
                                namespaceFactory.setNamespaceDetails($scope.nameSpace,
                                        $scope.nameSpace.name)
                                    .then(function(res) {
                                            //success
                                            $scope.editNamespaceFlag = true;
                    ...
                    if ($scope.createNameSpace.$valid) {
                        ...
                        arr.push($scope.nameSpace.name);
                        arr.push("on");
                        arr.push(moment().startOf($scope.nameSpace.createdOn).format(
                            'MMMM Do YYYY, h:mm:ss a'));
                        // $rootScope.socket1.emit('notification',arr);
                        $scope.error = "";
                ...
                for (var i in dataObj) {
                    ...
                    if ((typeof dataObj[i]) === 'object') {
                        ...
                        for (var j in dataObj[i]) {
                            ...
                            if (typeof dataObj[i][j] == 'object' && !(Array.isArray(dataObj[
                                    i][j]))) {
