...
router.get('/watchprocessors', function(req, res) {
  getWatchProcessorMap()
    ...
    .catch(function(err) {
      ...
      return res.status(500).json({
        error: "Internal error in obtaining watch processor details"
      });
    })
});
...
var saveWatchloopEntries = function(watchLoopEntry, orgsite) {
  return new Promise(function(resolve, reject) {
    ...
    if (watchLoopEntry.watchname === undefined) {
      reject({
        error: "Error in getting the watchname..!"
      });
    }

    var WatchListModel = dataProvider.getModel(WatchListSchema, orgsite);
    WatchListModel.findOne({
      name: watchLoopEntry.watchname
    }, function(err, watchlist) {
      if (err || (!watchlist)) {
        logger.debug("Unable to find the requested watch list: ", err);
        reject({
          error: "Unable to get the watchlist requested ",
        });
      }
      ...
      watchloopObj.save(function(err, savedWatchLoopObj) {
        if (err) {
          ...
          reject({
            error: "Internal error in starting with requested watch list execution..!"
          });
        }
...
var editWatchloopEntries = function(watchListEntry, orgsite) {
  return new Promise(function(resolve, reject) {
    ...
    WatchLoopModel.update({
      "watchname": watchname
    }, watchloopObj, function(err, updatedObj) {
      if (err) {
