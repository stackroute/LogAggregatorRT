...
function sourceConnectorTask(subscribeFrom, publishTo, payload) {
  ...
  this.doTask = function() {
    ...
    subChannelClient.on('message', function(channel, data) {
      ...
      var dataSourceObj = getDataSourceObj(payload.watch.stream, payload.watch.orgsite)
        .then(function(response){
          logger.debug("Connecting to data source: ", response.ipaddr, ":", response.port, " from ", subscribeFrom);
