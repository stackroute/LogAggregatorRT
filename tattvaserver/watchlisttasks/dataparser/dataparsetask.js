...
function dataParserTask(subscribeFrom, publishTo, payload) {
    ...
    this.doTask = function() {
        ...
        subChannelClient.on('message', function(channel, data) {
            ...
            if (data["data"]) {
                //If the incoming has data in another filed then
                data = data["data"];
            } else {
                //this is due to the way we are using git log, once format of it is fixed, we should remove this              
                data = data[2];
            }

            // logger.debug("** The Data taken for processing ", data);
